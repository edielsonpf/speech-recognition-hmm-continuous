...
********************************************************************************
*
	Name: recognition_continuous_full.c
...
algorithm and continuous models. This algorithm is described in the paper " A Tutorial on Hidden Markov Models and selected Applications on Speech Recognition " of L. R. Rabiner. ( february of 1989). It is considered full covariance 
matrix.
...
//#define MAX_TIME 				300     /* maximum time */
#define MAX_TIME 				1000     /* maximum time */
...
{   /* mixture */
       ...
       double cov_matrix[MAX_COEF_NUMBER][MAX_COEF_NUMBER]; /* mixture full covariance matrix */
...
int main(int argc, char *argv[])
{
	...
	char data_file[MAX_PARAMETERS_NUMBER][100]; /* name of file with parameters */
	...
	char   date_time_f[MAX_WORD_STRING_SIZE];          /* variable to store formatted time and date */
	...
	if (argc  < 7)
	{
		puts("Usage: recognition_continuous_full models_number  model1 ... modelN coef_model1 ... coef_modelN input_file1 ... input_fileM  word_file output_file");
	...
	strcpy(output_file,argv[argc-1]);  /* file where the results will be written */
	strcpy(word_file,argv[argc-2]);    /* file with the spoken words */
	...
	for (i=0;i<models_number;i++)
	{
		 ...
		 while (fscanf(f_in1,"%s",model_name) != EOF)
		 {
				...
				if (first_model[i] == NULL)	first_model[i] = model_ptr;
				else	aux_ptr->next = model_ptr;
	...
	strncpy(last_word," ",MAX_WORD_STRING_SIZE);  /* last word recognized */
	...
	while (fscanf(f_in1,"%s",spoken_word) != EOF ) /* testing the end of file of testing sequence */
	{
		if (ferror(f_in1))
		{
		   printf("reading error on file %s \n",word_file);
...
//		printf("\n%s = %s",last_word,spoken_word);
		if (strncmp(last_word,spoken_word,MAX_WORD_STRING_SIZE) != 0 )
		{
		   if(strncmp(last_word," ",MAX_WORD_STRING_SIZE) != 0)
		   {
			  /* computing cpu time */
			  times(&aux_time);       /* getting cpu time */
			  aux = (aux_time.tms_utime)/60.0;
		...
		for (j=0;j<models_number;j++)
		{
			 ...
			 for (k=0;k<word_number;k++)
			 {
				  ...
				  printf("\r\nCalculating Alpha");
				  calc_alpha(model_ptr->states_number,obs_time,model_ptr->param_number,
							 alpha,scaling_factor,model_ptr->transition_probab,
							 symbol_probab,pi);
				  probab[k] += coef_model[j]*calc_probability(obs_time,	scaling_factor);
		...
		if (strcmp(spoken_word,word[index[0]]) == 0)	correct++;
		else
		{
			  error++;
			  wrong_word[index[0]]++;
			  if (strcmp(spoken_word,word[index[1]]) == 0)	  second++;
		}
	...
	/* writing results */
	writing_result_word(correct,error,second,word_number,last_word,wrong_word,
...
//		printf("\r\nState %d:",i);
		...
		{
//			printf("\r\nSymbol prob: %f:",symbol_probab[j][i][0]);
...
//	printf("\r\nsum: %f, \r\nscaling_factor: %f",sum, scaling_factor[0]);
...
double calc_probability(obs_time,scaling_factor)
int obs_time;
double scaling_factor[MAX_TIME];
...
//				printf("\r\ninv: %f, result=%f",inv_cov[j][i],dif[j]*inv_cov[j][i]);
			}
			aux += dif[i]*temp;
		}
//		printf("\r\ntemp=%f",temp);
//		printf("\r\naux = %f, aux*-0.5=%f, exp(aux)=%f",aux,aux*(-0.5),exp(aux));
		aux *= (-0.5);
		aux = exp(aux);
		gaus = aux/(aux1*aux2);
	}
...
double *coef_model;      /* weighting model coefficients */
char *argv[];
...

{
	...
	fprintf(f_out,"Isolated word recognition using Continuous HMM. It is considered full covariance matrix.\n");
	fprintf(f_out,"Algorithm used for recognition: Forward \n");
	fprintf(f_out,"Number of models: %d  \n",models_number);
	for (i=0;i<models_number;i++)
	{
		 fprintf(f_out,"Model name %d: %s\n",(i+1),argv[2+i]);
		 fprintf(f_out,"Weighting coefficient of model %d:%.2f\n",(i+1),coef_model[i]);
	}
...
void writing_word(char *word)   /* spoken word */
{
	fprintf(f_out,"\nSpoken word: %s\n",word);
